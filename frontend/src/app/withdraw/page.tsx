        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <div className="space-y-2">
              {withdrawals.length === 0 ? <p className="text-sm" style={{ color: "var(--foreground-muted)" }}>No withdrawals yet</p> : withdrawals.slice(0, 10).map(w => (
                <div key={w.id} className="flex justify-between py-2 border-b border-gray-100 last:border-0">
                  <div>
                    ...
                    <p className="text-xs" style={{ color: "var(--foreground-muted)" }}>{formatDate(w.createdAt)}</p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full ${w.status === 'COMPLETED' ? 'bg-emerald-500/10 text-emerald-400' : w.status === 'REJECTED' ? 'bg-red-500/10 text-red-400' : 'bg-amber-500/10 text-amber-400'}`}>{w.status}</span>
                </div>
              ))}
            </div>
          </Card>
